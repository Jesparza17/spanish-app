...
export type ReviewMode =
  | { type: "frequency" } // default SRS order, all due items
...
export async function fetchDueQueue(
  ...
  if (itemKind === "vocab" || itemKind === "both") {
    cards.push(...(await fetchDueVocab(userId, mode)));
  }
  if (itemKind === "verb" || itemKind === "both") {
    cards.push(...(await fetchDueVerbs(userId, mode)));
  }

  return cards.slice(0, MAX_QUEUE_SIZE);
}

async function fetchDueVocab(userId: string, mode: ReviewMode): Promise<ReviewCard[]> {
  let query = supabase
    .from("srs_state")
    .select(
      `id, due_at, vocab_items!inner(id, lemma, translation, example_sentence, example_translation, cefr_level${
...
async function fetchDueVerbs(userId: string, mode: ReviewMode): Promise<ReviewCard[]> {
  let query = supabase
    .from("srs_state")
    .select(
      `id, due_at, verbs!inner(id, infinitive, translation, example_sentence, example_translation, cefr_level${
